  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <div className="p-3 rounded-2xl bg-slate-100 border border-slate-200 group-hover:scale-110 transition-transform duration-500">
          <Icon size={24} />
        </div>
      </div>
      <div>
        <p className="text-slate-600 text-sm font-semibold tracking-wide uppercase">{title}</p>
        <h3 className="text-xl lg:text-2xl font-black mt-2 text-slate-900 break-words">{value}</h3>
      </div>
    </motion.div>
  );
};
...
export default function Dashboard() {
  ...
  return (
    <div className="flex bg-[#f8fafc] min-h-screen text-slate-900 selection:bg-blue-500/30">
      <Sidebar />
      <main className="flex-1 p-10 overflow-y-auto">
        ...
        >
          ...
          <h1 className="text-5xl font-black text-slate-900 tracking-tight">Stock Command Center</h1>
          <p className="text-slate-600 mt-3 text-lg font-medium max-w-2xl">Monitor real-time inventory flow and manage your catalog with precision.</p>
        </motion.header>
        ...
        >
          <form onSubmit={handleStockAction} className="space-y-5">
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-600 uppercase tracking-widest px-1">Target Product</label>
              <select 
                required
              ...
              >
                <option value="" className="bg-white text-slate-900">Choose a product...</option>
                {products.map(p => (
                  <option key={p.id} value={p.id} className="bg-white text-slate-900">{p.name} ({p.sku})</option>
                ))}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-600 uppercase tracking-widest px-1">Quantity to Adjust</label>
              <input 
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-600 uppercase tracking-widest px-1">Transaction Note</label>
              <input 
                className="input-field"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-4">
              {loading ? (
                <div className="flex flex-col items-center py-20 gap-4">
                  ...
                  <p className="text-slate-600 font-bold uppercase tracking-widest text-xs">Syncing data...</p>
                </div>
              ) : stats.recentActivities.length === 0 ? (
                <p className="text-slate-600 text-center py-20 font-medium">No movements recorded yet.</p>
              ) : (
                stats.recentActivities.map((activity: any, idx: number) => (
                  <motion.div 
                    ...
                    key={activity.id} 
                    className="flex items-center justify-between p-5 rounded-2xl bg-white border border-slate-200 hover:border-blue-200 hover:bg-blue-50 transition-all group/item"
                  >
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <p className="font-black text-slate-900 group-hover/item:text-blue-600 transition-colors">{activity.product_name}</p>
                        <p className="text-xs text-slate-600 font-bold uppercase mt-1 tracking-tighter">
                        ...
                        {activity.notes && (
                          <p className="text-[10px] text-slate-500 italic mt-1 line-clamp-1 max-w-[200px]">"{activity.notes}"</p>
                        )}
                      </div>
                    </div>
          ...
          <div className="space-y-8">
            ...
            >
              <h2 className="text-2xl font-black text-slate-900 mb-8">Stock Ops</h2>
              <div className="grid grid-cols-1 gap-4">
                <button 
                  onClick={() => { setStockAction('IN'); setIsStockModalOpen(true); }}
                  className="flex items-center justify-between p-5 rounded-2xl bg-emerald-100 border border-emerald-300 text-emerald-700 hover:bg-emerald-500 hover:text-white transition-all duration-300 font-black group"
                >
                ...
                <button 
                  ...
                  className="flex items-center justify-between p-5 rounded-2xl bg-rose-100 border border-rose-300 text-rose-700 hover:bg-rose-500 hover:text-white transition-all duration-300 font-black group"
                >
                  <div className="flex items-center gap-4">
